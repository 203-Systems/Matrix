  ...
  if (KeyPad.scan())
  {
    ...
    if(fn_hold)
    {
      if(KeyPad.fn.changed)
      {
        ...
        else if(KeyPad.fn.state = HOLD)
        {
          Midi.sentNoteOff(0, keymap_fn[current_keymap]);
          UI.enterFNmenu();
          KeyPad.fn.activeTime = 0xFFFFFFFF; //Prevent back to FN
        }
      }
    }
    else
